...
final <-
  Q19 |>
  mutate(
    final_grade = rowMeans(across(!c(id, group, sex)),
      na.rm = TRUE
    ))|>
  select(final_grade,id,group,sex) |> 
...
```{r}
ggplot(final, aes(final_grade)) +
  geom_histogram(binwidth = 0.5) +
  facet_wrap(~group)+
  theme_clean() +
  labs(
    title = "Differences in final grades between groups"
  )
```
...
```{r}
ggplot(final, aes(final_grade, fill = sex)) +
  geom_histogram(binwidth = 0.5) +
  facet_wrap(~group)+
  theme_clean() +
  labs(
...
Q26 <-
  ...
  mutate(
    pass_final = if_all(
      !c(id, group, sex,final_grade),     
      ~ .x > 5
    )
  )|>
  mutate(pass_final = if_else(final_grade > 10, "Pass", "Fail"))
...
Q27 <-
  Q26 |> 
  filter(final_grade >= 10 & pass_final == "Fail")
```

there is `r nrow(Q27)` such person
...
Q28 <-
  Q26 |> 
  summarise(
    pass_rate = mean(pass_final == "Pass"),
    .by = group
  )

ggplot(Q28, aes(group, pass_rate)) +
  geom_col() +
